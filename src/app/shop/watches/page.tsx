...
export default function WatchesPage() {
  const catProducts = products.filter((p) => p.category === "Watches");
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="flex items-center justify-center gap-2 mb-3">
              <Watch className="w-5 h-5 text-[#CFF227]" />
              <p className="text-[#CFF227] text-[10px] tracking-[0.5em] uppercase font-black">COLOUR SEVEN FASHION</p>
            </div>
            <h1 className="font-serif text-5xl md:text-7xl uppercase tracking-widest skew-x-[-6deg]">WATCHES</h1>
            <div className="h-[2px] w-16 bg-[#CFF227] mx-auto mt-4 rounded-full" />
            <p className="text-zinc-500 text-xs tracking-[0.25em] uppercase font-bold mt-4">TIMELESS PRECISION — {catProducts.length} ITEMS</p>
          </motion.div>
        </div>
      </div>
